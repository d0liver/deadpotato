...
# case)
fixData = () ->
	window.cnt.countries = _.map cnt.countries, (country) ->
		country.color = country.color.toLowerCase()
		country

	window.map_data.spaces = for space in map_data.spaces
		space.abbreviations = (abbr.toLowerCase() for abbr in space.abbreviations)
		space.name = space.name.toLowerCase()
		space

	window.rgns = _.object _.map rgns, (rgn, rgn_name) ->
		return [rgn_name.toLowerCase(), rgn]

	window.gam.country_infos =
		gam.country_infos.map (country_info) ->
			country_info.units = country_info.units.map (unit) -> unit.toLowerCase()
			country_info.supply_centers =
				country_info.supply_centers.map (center) -> center.toLowerCase()
