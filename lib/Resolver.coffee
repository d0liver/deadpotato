...
Resolver = (engine, gd) ->
	...
	self.adjudicate = (order) ->
		...
		switch order.type
			when 'MOVE'
				...
				preventers = gd.ordersWhere(order, 'MOVE', 'EXISTS', to: order.to) or []
				...
				[opposing_order] = gd.ordersWhere(order, 'MOVE', 'EXISTS',
					to: order.from
					from: order.to
				) ? []
			...
			when 'SUPPORT'
				...
				return ! gd.ordersWhere order, 'MOVE', 'EXISTS',
					to: order.supporter
					from: (f) ->
						f isnt order.to
					country: (c) -> c isnt order.country
			...
			when 'CONVOY'
				...
				return ! gd.ordersWhere order, 'MOVE', 'SUCCEEDS', to: order.convoyer

	holdStrength = (region) ->
		oW = gd.ordersWhere.bind(gd, null)
	...
	attackStrength = (order) ->
		oW = gd.ordersWhere.bind(gd, order)
	...
	preventStrength = (order) ->
		...
		# strength is 0
		if gd.ordersWhere(order, 'MOVE', 'SUCCEEDS', to: order.from)
			return 0
		else
			return 1 + support(order)
	...
	hasPath = (order) ->
		...
		unless \
		order.type in 'MOVE' or
		order.type is 'CONVOY' and self.adjudicate(order)
			return false

		# Base case - Destination is adjacent
		if engine.regionIsAdjacent(actor(order), order.to)
			return true

		# Recursive case - The destination is not adjacent but there is at
		# least one convoy.
		convoys = engine.adjacentConvoys(order)
		for convoy in convoys when hasPath(convoy)
			return true
	...
	support = (order) ->
		{from, to} = order
		return gd.ordersWhere(order, 'SUPPORT', 'SUCCEEDS', {from, to})?.length ? 0
